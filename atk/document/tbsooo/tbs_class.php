...
function tbs_Locator_PrmRead(&$Txt,$Pos,$HtmlTag,$DelimChrs,$BegStr,$EndStr,&$Loc,&$PosEnd) {
  ...
  // Param�tres de v�rif de la boucle
  ...
  while ($Continue) {
    ...
    if ($DelimIdx) { // Lecture dans une cha�ne

      if ($Chr===$DelimChr) { // Quote rencontr�
        if ($Chr===$Txt[$Pos+1]) { // Double quote => la cha�ne continue en d�doublant le quote
          $Pos++;
        } else { // Simple quote => fin de la cha�ne
          $DelimIdx = false;
        }
      }

    } else { // Lecture hors cha�ne
      
      if ($BegCnt===0) {
        
        // Analyse des param�tre
        $CheckChr = false;
        if ($Chr===' ') {
          if ($Status===1) {
            $Status = 2;
