...
function showTab(tab)
{
  ...
  $$('div', 'div.atkSection').select(isAttributeTr).each(function(tr) {
    var visible =
	...
	for(j = 0; j < tabs.length; j++)
	{
	  if(document.getElementById('tab_'+tabs[j]))
      {
		if(tabs[j] == tab)
		{
			document.getElementById('tab_'+tabs[j]).className = 'nav-item active';
      document.getElementById('tab_'+tabs[j]).firstElementChild.classList.add('active');
		}
		else
		{
		  document.getElementById('tab_'+tabs[j]).className = 'nav-item';
      document.getElementById('tab_'+tabs[j]).firstElementChild.classList.remove('active');
		}
	 }
	}
