   ...
   * $Id: formselect.js 6418 2009-07-02 09:03:12Z patrick $
   */
...
var len=(list.length);
  ...
  for (var i = 0; i < list.length; i++)
  {
    if(list[i].checked==true){n++;}
  } 

 
    if(n>0 && n<len){$('mrasection').show();$('mracheck').checked=false;}
    if(n==0){$('mracheck').checked=false;$('mrasection').hide();}
    if(n==len){$('mracheck').checked=true;$('mrasection').show();}
...
function atkSubmitMRPA(name, form, target)
{
   ...
   * $Id: formfocus.js 6407 2009-06-19 10:12:02Z peter $
   */
